...
static uint8_t merchant_in_known_list(const transaction_context *self) {
...
static bool transaction_context__parse(transaction_context *self, const char *body,
                                       size_t body_len) {
...
void transaction_context__destroy(transaction_context *self) {
  if (self == NULL) {
    return;
  }
...
transaction_context transaction_context__new(void) {
...
transaction_context transaction_context__from_body(const char *body, size_t body_len) {
  transaction_context ctx = transaction_context__new();
  transaction_context__parse(&ctx, body, body_len);
  return ctx;
}

void transaction_context__to_vector(const transaction_context *self, double out[14]) {
  double amount = 0.0;
